/*
----------------------------------------------------------------------------------------------------------------
-- PROJET DE GESTION DE LA COMPAGNIE AERIENNE AVEC LE SGBD MySQL
-- GROUPE 2 
----------------------------------------------------------------------------------------------------------------
...
*/
USE `agence_voyage_sgbd`;

/*-----------------------------------------------------------------------------------------------------
-- Création de l'utilisateur avec accord de tous les privilèges
*/
CREATE USER 'compagnie'@'localhost' IDENTIFIED BY 'compagnie';

GRANT ALL PRIVILEGES
ON `agence_voyage_sgbd`.*
TO 'compagnie'@'localhost';
...
/*-----------------------------------------------------------------------------------------------------
-- 1)	Afficher la liste des clients qui ont programmé un séjour par date
*/
SELECT nom_client 
FROM clients, reservations 
WHERE clients.client_id = reservations.client_id 
ORDER BY date_debut;

/*-----------------------------------------------------------------------------------------------------
-- 2)	Afficher la liste des clients les plus fidèles, avec pour chaque client la liste des séjours consommé
*/
SELECT clients.nom_client, COUNT(reservations.reservation_id) AS nombre_sejours
FROM clients, reservations
WHERE clients.client_id = reservations.client_id
...
/*-----------------------------------------------------------------------------------------------------
-- 3)	Afficher l'hôtel qui a fait l'objet de plus de commandes
*/
SELECT hotels.nom_hotel, COUNT(reservations.reservation_id) AS nombre_reservations
FROM hotels, reservations
WHERE hotels.hotel_id = reservations.hotel_id
GROUP BY hotels.nom_hotel
ORDER BY nombre_reservations DESC
LIMIT 1;

/*-----------------------------------------------------------------------------------------------------
-- 4)	Afficher le séjour le plus commandé de l'année
*/
SELECT sejours.nom_sejour, COUNT(reservations.reservation_id) AS nombre_reservations
FROM sejours, reservations
WHERE sejours.sejour_id = reservations.reservation_id
  AND YEAR(reservations.date_debut) = YEAR(CURDATE())
GROUP BY sejours.nom_sejour
ORDER BY nombre_reservations DESC
LIMIT 1;